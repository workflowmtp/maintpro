...
export async function POST() {
  try {
    ...
    for (const a of ateliers) {
      await prisma.atelier.create({ data: a });
    }
    ...
    for (const o of operateurs) {
      await prisma.operateur.create({ data: o });
    }
    ...
    for (const c of chefs) {
      await prisma.chefAtelier.create({ data: c });
    }
    ...
    for (const c of causes) {
      await prisma.cause.create({ data: c });
    }
    ...
    for (const o of organes) {
      await prisma.organe.create({ data: o });
    }
    ...
    for (const p of pieces) {
      await prisma.piece.create({ data: p });
    }

    // Date helpers
    const now = new Date();
    const y = now.getFullYear(), mo = now.getMonth();
    const d = (day: number, hour: number = 8) => new Date(y, mo, day, hour).toISOString();
    ...
    for (const da of das) {
      await prisma.demandeAchat.create({ data: da });
    }

    // --- SOUS-TRAITANCES ---
    await prisma.sousTraitance.create({
      data: {
    ...
    for (const t of taches) {
      await prisma.tachePreventive.create({ data: t });
    }

    // --- UTILISATEURS (avec email + mot de passe hashé) ---
    const users = [
      { id: 'usr_01', nom: 'Admin Systeme', email: 'admin@maintpro.cm', password: hash('admin1234'), login: 'admin', role: 'admin', pole_id: null, actif: true },
      { id: 'usr_02', nom: 'Directeur General', email: 'dg@maintpro.cm', password: hash('dg1234'), login: 'dg', role: 'direction', pole_id: null, actif: true },
      { id: 'usr_03', nom: 'Ekani Robert', email: 'rekani@maintpro.cm', password: hash('rekani1234'), login: 'rekani', role: 'resp_maintenance', pole_id: null, actif: true },
      { id: 'usr_04', nom: 'Abanda Louis', email: 'labanda@maintpro.cm', password: hash('labanda1234'), login: 'labanda', role: 'resp_pole', pole_id: 'pole_oe', actif: true },
      { id: 'usr_05', nom: 'Messi Roger', email: 'rmessi@maintpro.cm', password: hash('rmessi1234'), login: 'rmessi', role: 'chef_atelier', pole_id: 'pole_oe', actif: true },
      { id: 'usr_06', nom: 'Mbarga Jean', email: 'jmbarga@maintpro.cm', password: hash('jmbarga1234'), login: 'jmbarga', role: 'technicien', pole_id: 'pole_oe', actif: true },
      { id: 'usr_07', nom: 'Ondoua Martin', email: 'mondoua@maintpro.cm', password: hash('mondoua1234'), login: 'mondoua', role: 'operateur', pole_id: 'pole_oe', actif: true },
      { id: 'usr_08', nom: 'Ngo Biyick Anne', email: 'ango@maintpro.cm', password: hash('ango1234'), login: 'ango', role: 'magasinier', pole_id: null, actif: true },
      { id: 'usr_09', nom: 'Fotso Albert', email: 'afotso@maintpro.cm', password: hash('afotso1234'), login: 'afotso', role: 'technicien', pole_id: 'pole_hf', actif: true },
      { id: 'usr_10', nom: 'Fouda Michel', email: 'mfouda@maintpro.cm', password: hash('mfouda1234'), login: 'mfouda', role: 'chef_atelier', pole_id: 'pole_bc', actif: true },
    ];
    ...
    for (const s of signalements) {
      await prisma.signalement.create({ data: s });
    }
    ...
    return NextResponse.json({
      ...
      seeded: true,
      users_info: 'Mot de passe par defaut: {login}1234 (ex: admin1234, dg1234, rekani1234...)',
    });
  } catch (error: any) {
